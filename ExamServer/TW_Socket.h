...
/* 
    아래의 8개 함수는 문자열의 인코딩 형식을 변환하는 함수입니다.
	이름이 TWAPI_ AAA to BBB 형식인데 AAA 형식의 문자열을 BBB 형식으로 문자열로 변환한다는 뜻임
	예를 들어, TWAPI_UnicodeToAscii 함수는 유니코드 형식의 문자열을 아스키코드 형식의 문자열로 변환한다는 뜻임
	그래서 ap_src_string에 저장된 문자열이 ap_dest_string 으로 변환되어 저장되는 형식임
	ap_dest_string이 1차원 포인터인 경우에는 함수를 호출하는 쪽에서 메모리를 할당해서 넘길때 사용하고
	ap_dest_string이 2차원 포인터인 경우에는 함수 내부에서 메모리를 할당하고 문자열을 변환합니다. 따라서 함수를 호출한 쪽에서
	반드시 메모리를 해제해야 합니다.
	...
	그리고 함수의 반환 값은 변환된 문자열의 길이인데 '\0'까지 포함된 길이 입니다. "abc"이면 4가 반환됩니다.
*/
